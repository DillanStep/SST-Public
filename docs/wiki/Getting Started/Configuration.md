...
1. The DayZ mod writes data under the active DayZ profile folder.
...
SST writes to `$profile:SST`, so `SST_PATH` is the `SST` folder inside the active DayZ `-profiles` directory. The DayZ script log prints this path on startup as `Runtime path ready`.
...
If you are unsure, use the folder that contains `api/online_players.json` after the server has started with `@SST`.
