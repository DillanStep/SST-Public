...
echo Building SST.pbo...
"%ADDON_BUILDER%" "%SOURCE%" "%BUILD_DIR%" -packonly -clear -prefix=SST -project="%PROJECT%" -toolsDirectory="%DAYZ_TOOLS%"
if errorlevel 1 (
...
echo Updating local @SST package...
call :BackupFile "%ADDONS%\SST.pbo"
call :BackupFile "%ADDONS%\Scripts.pbo"

if exist "%ADDONS%\Scripts.pbo" (
	echo Removing old ignored Scripts.pbo from local @SST package...
	del /F /Q "%ADDONS%\Scripts.pbo"
)

copy /Y "%BUILT_PBO%" "%ADDONS%\SST.pbo" >nul
if errorlevel 1 (
	echo ERROR: Could not copy built PBO into "%ADDONS%".
	exit /b 1
)

echo Local package updated: "%ADDONS%\SST.pbo"
...
if not "%SERVER_ROOT%"=="" (
	...
	echo   build mod.bat "D:\SteamLibrary\steamapps\common\DayZServer Development Server"
)
...
:InstallToServer
set "TARGET_ROOT=%~1"
set "TARGET_MOD=%TARGET_ROOT%\@SST"
...
if not errorlevel 1 (
	...
	echo The local @SST package has been rebuilt, but the server copy was not changed.
	exit /b 1
)
...
echo.
echo Replacing server @SST package: "%TARGET_MOD%"
call :BackupFile "%TARGET_ADDONS%\SST.pbo"
call :BackupFile "%TARGET_ADDONS%\Scripts.pbo"

if exist "%TARGET_ADDONS%\Scripts.pbo" (
	echo Removing old ignored Scripts.pbo from server @SST package...
	del /F /Q "%TARGET_ADDONS%\Scripts.pbo"
)

robocopy "%PACKAGE%" "%TARGET_MOD%" /E /NFL /NDL /NJH /NJS /NC /NS >nul
if errorlevel 8 (
	echo ERROR: Failed to copy @SST to "%TARGET_MOD%".
	exit /b 1
)
