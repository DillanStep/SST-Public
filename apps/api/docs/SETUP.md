...
Start the DayZ server once with `@SST` loaded before testing API paths. The mod must create the `$storage:SST` folder first.
...
# DayZ paths (required)
# SST_PATH is generated by @SST under DayZ $storage.
# Recommended: set DayZ -storage=Server1Storage and point here:
...
|----------|-------------|---------|
| `SST_PATH` | SST mod data folder generated under `$storage:SST` | `C:/DayZServer/Server1Storage/SST` |
...
│   └── ExpansionMod/
├── Server1Storage/             ← storage root when -storage=Server1Storage
│   └── SST/                    ← SST_PATH
│       ├── api/
│       ├── inventories/
│       ├── events/
