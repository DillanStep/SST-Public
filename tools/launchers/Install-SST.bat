...
if not exist "%API_DIR%\package.json" (
    ...
    echo Make sure the SST folder structure is intact.
    pause
    exit /b 1
)
...
echo.
echo   1. Double-click "Start-SST.bat" in the SST root,
echo      or run tools\launchers\Start-SST.bat
echo.
...
echo.
echo   NOTE: Copy dayz\server-mod\@SST to the DayZ server and start the
