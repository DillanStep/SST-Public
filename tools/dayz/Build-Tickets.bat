...
set "SOURCE=%ROOT%dayz\mod-source\SST_Tickets"
set "PACKAGE=%ROOT%dayz\server-mod\@SST_Tickets"
set "ADDONS=%PACKAGE%\Addons"
set "BUILD_DIR=%ROOT%build\pbo"
set "BACKUP_DIR=%ROOT%build\backups"
set "BUILT_PBO=%BUILD_DIR%\SST_Tickets.pbo"

for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd-HHmmss"') do set "STAMP=%%I"

echo.
echo === SST Tickets mod build ===
...
echo Building SST_Tickets.pbo...
"%ADDON_BUILDER%" "%SOURCE%" "%BUILD_DIR%" -packonly -clear -prefix=SST_Tickets -project="%PROJECT%" -toolsDirectory="%DAYZ_TOOLS%"
if errorlevel 1 (
...
echo Updating local @SST_Tickets package...
call :BackupFile "%ADDONS%\SST_Tickets.pbo"

copy /Y "%BUILT_PBO%" "%ADDONS%\SST_Tickets.pbo" >nul
if errorlevel 1 (
	echo ERROR: Could not copy built PBO into "%ADDONS%".
	exit /b 1
)

echo Local package updated: "%ADDONS%\SST_Tickets.pbo"
...
if not "%SERVER_ROOT%"=="" (
	...
	echo.
	echo No server path supplied, so only the project @SST_Tickets package was updated.
	echo To also replace a server install, run:
	echo   tools\dayz\Build-Tickets.bat "C:\DayZServer"
)
...
:InstallToServer
set "TARGET_ROOT=%~1"
set "TARGET_MOD=%TARGET_ROOT%\@SST_Tickets"
...
if not errorlevel 1 (
	...
	echo The local @SST_Tickets package has been rebuilt, but the server copy was not changed.
	exit /b 1
)
...
echo.
echo Replacing server @SST_Tickets package: "%TARGET_MOD%"
call :BackupFile "%TARGET_ADDONS%\SST_Tickets.pbo"

robocopy "%PACKAGE%" "%TARGET_MOD%" /E /NFL /NDL /NJH /NJS /NC /NS >nul
if errorlevel 8 (
	echo ERROR: Failed to copy @SST_Tickets to "%TARGET_MOD%".
	exit /b 1
)
