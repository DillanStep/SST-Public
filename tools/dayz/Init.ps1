...
$DayzRoot = Join-Path $RepoRoot 'dayz'
$current_item = Get-Item (Join-Path $DayzRoot 'workbench-template')
...
New-Item -Path (Join-Path $DayzRoot "missions\Global") -ItemType Directory
...
# Run Setupworkdrive
& (Join-Path $ScriptDir 'SetupWorkdrive.bat')
