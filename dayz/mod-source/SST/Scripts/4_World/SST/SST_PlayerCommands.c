...
class SST_PlayerCommands
{
	...
	static const float DEFAULT_BLOOD_MAX = 5000.0;
	static const float DEFAULT_SHOCK_MAX = 100.0;
	...
	protected void ProcessHealCommand(SST_PlayerCommandRequest request, PlayerBase player, string playerName)
	{
		...
		float maxBlood = player.GetMaxHealth("GlobalHealth", "Blood");
		if (maxBlood <= 0)
			maxBlood = DEFAULT_BLOOD_MAX;
		player.SetHealth("GlobalHealth", "Blood", maxBlood * multiplier);
	...
	protected void RestoreShock(PlayerBase player)
	{
		float maxShock = player.GetMaxHealth("", "Shock");
		if (maxShock <= 0)
			maxShock = player.GetMaxHealth("GlobalHealth", "Shock");
		if (maxShock <= 0)
			maxShock = DEFAULT_SHOCK_MAX;

		player.SetHealth("", "Shock", maxShock);
		player.SetHealth("GlobalHealth", "Shock", maxShock);
	}
