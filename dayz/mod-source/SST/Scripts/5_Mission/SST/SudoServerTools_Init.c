...
modded class MissionServer
{
	override void OnInit()
	{
		...
		if (GetGame().IsServer())
		{
			...
			Print("[SST] MissionServer.OnInit - Starting REST Event Client");
			SST_RestEventClient.Start();
			...
			// Start Expansion AI tracker when Expansion AI is loaded
			Print("[SST] MissionServer.OnInit - Starting Expansion AI Tracker");
			SST_ExpansionAITracker.Start();
			#endif
			...
			Print("[SST] MissionServer.OnInit - Starting Expansion ATM Commands");
			SST_ExpansionATMCommands.Start();
			#endif
...
#ifdef EXPANSIONAI_ONFACTIONCHANGE
// ============================================================================
// Expansion AI Tracker - Tracks live DayZ Expansion AI locations
// ============================================================================
class SST_ExpansionAITracker
{
	protected static ref SST_ExpansionAITracker s_Instance;
	protected bool m_Initialized;

	static const float UPDATE_INTERVAL = 10000.0; // 10 seconds
	static string AI_POSITIONS_FILE = SST_RuntimePaths.ApiFile("ai_positions.json");

	static SST_ExpansionAITracker GetInstance()
	{
		if (!s_Instance)
			s_Instance = new SST_ExpansionAITracker();
		return s_Instance;
	}
	...
	protected void Init()
	{
		...
		m_Initialized = true;
		Print("[SST] ExpansionAITracker initializing...");

		SST_PersistenceCore.EnsureDirectory(SST_RuntimePaths.STORAGE_ROOT);
		SST_PersistenceCore.EnsureDirectory(SST_RuntimePaths.API_FOLDER);

		GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(UpdateAndScheduleNext, 3000, false);
		Print("[SST] Expansion AI Tracker started - updating every 10 seconds");
	}
	...
	protected void ExportAIPositions()
	{
		if (!GetGame() || !GetGame().IsServer())
			return;

		ref SST_AIPositionsData exportData = new SST_AIPositionsData();
		exportData.generatedAt = SST_OnlinePlayerTracker.GetUTCTimestamp();
		exportData.modVersion = SST_ModInfo.VERSION;
		exportData.protocolVersion = SST_ModInfo.PROTOCOL_VERSION;

		array<eAIBase> aiUnits = eAIBase.eAI_GetAll();
		if (aiUnits)
		{
			for (int i = 0; i < aiUnits.Count(); i++)
			{
				eAIBase unit = aiUnits.Get(i);
				if (!unit || !unit.IsAlive())
					continue;

				ref SST_AIPositionData aiData = BuildAIPosition(unit, i);
				if (!aiData)
					continue;

				exportData.ai.Insert(aiData);
			}
		}

		exportData.aiCount = exportData.ai.Count();

		string errorMsg;
		if (!SST_Persistence<SST_AIPositionsData>.SaveJson(AI_POSITIONS_FILE, exportData, errorMsg))
		{
			Print("[SST] ERROR: Failed to save Expansion AI positions: " + errorMsg);
		}
	}

	protected SST_AIPositionData BuildAIPosition(eAIBase unit, int index)
	{
		if (!unit)
			return null;

		vector pos = unit.GetPosition();
		string typeName = unit.GetType();
		string factionName = "";
		string groupName = "";
		...
		ref SST_AIPositionData aiData = new SST_AIPositionData();
		aiData.aiId = "expansion-ai-" + index.ToString();
		aiData.displayName = typeName;
		aiData.typeName = typeName;
		aiData.faction = factionName;
		aiData.groupName = groupName;
		aiData.lastUpdate = SST_OnlinePlayerTracker.GetUTCTimestamp();
		aiData.posX = pos[0];
		aiData.posY = pos[1];
		aiData.posZ = pos[2];
		aiData.health = unit.GetHealth("GlobalHealth", "Health");
		aiData.isAlive = unit.IsAlive();
		aiData.isUnconscious = unit.IsUnconscious();
